...
void FillContainer_Bkg(TString ext[], TString ps[], TString suf)
{
  ...
  for (int i=0; i<N_ext; i++)
    {
      if (N_ext==0) break;
      TString filename = dir + ext[i] + "/modcube_M31_" + ext[i] + suf + ".fits";
  ...
  for (int i=0; i<N_ps; i++)
    {
      if (N_ps==0) break;
      TString filename =  dir_PS + "/modcube_M31_" + ps[i] + suf + ".fits";
      VM data;
      N_bkg.push_back(ReadFits(data,filename));
      Bkg_model.push_back(data);
    }
...
void FillContainer_DM(Number dm_mass, TString particle, TString suf)
{
  ...
  if (dm_mass < 1.0)
    ...
    TString modelname = "dm_M31_" + particle + masstr;
...
void FillContainer_Obs(TString obsname, bool modcube, TString suf)
{
  ...
  if (modcube)
    {
      filename = dir + obsname + "/modcube_M31_" + obsname + suf + ".fits";
    }
  else
    {
      filename = dir + obsname + "/cntcube_M31_" + obsname + suf + ".fits";     
    }
